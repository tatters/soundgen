...
Random pitch drift is added whenever temperature > 0. Use `tempEffects` to regulate its amount and frequency:
```{r fig.width = 5, fig.height = 5}
...
Note that rapid pitch drift may sound like jitter (see the section on nonlinear effects), but jitter is random variation around a target value, whereas pitch drift is implemented as a random walk, which can wander into values quite far removed from the target. Both pitch drift and jitter can also be combined.
...
```{r fig.width = 7, fig.height = 3}
estimateVTL(formants = c(500, 1500, 2550), plot = TRUE)  
# 17.5 cm
```
