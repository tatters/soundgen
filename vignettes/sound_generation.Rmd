...
```{r}
playback = c(TRUE, FALSE, 'vlc', 'my-awesome-player')[2]  
...
```{r fig.width = 7, fig.height = 4}
soundgen(formants = NULL, vocalTract = c(12, 18), play = playback, plot = TRUE)
```

If you specify both a few formant frequencies and variable "vocalTract", formant frequencies are exactly as specified at the initial value of "vocalTract" (that is, at the very beginning of the sound) and can deviate afterwards as VTL changes. Careful with such combinations: make sure the specified VTL is reasonable for these formant frequencies. Examples:
```{r fig.width = 7, fig.height = 4}
soundgen(sylLen = 800, pitch = 200,
         formants = list(f1 = c(500, 250), f2 = c(1500, 2800), f3 = 3500, f4 = 4300), 
         vocalTract = list(time = c(0, .3, 1), value = c(12, 16, 15)), 
         play = playback, plot = TRUE)
```
