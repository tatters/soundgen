# TODO: setting the intercept at 0 at estimateVTL makes the assumption of closed-open; for closed-closed, this assumption doesn't hold, so perhaps we don't set the intercept to 0; morph multiple sounds not just 2; check rw_smoothing in source, document and check if satisfied; check presets / demos to make sure formants are fine; maybe vectorize lipRad/noseRad; some smart rbind_fill in all ...Folder functions() in case of missing columns; soundgen- use psola when synthesizing 1 gc at a time; gaussian wn implemented in seewave (check updates!); soundgen - pitch2 for dual source (desynchronized vocal folds); AM aspiration noise (not really needed, except maybe for glottis > 0); soundgen() should accept smth like pitch = c(300, NA, 150, 250) and interpret this as two syllables with a pause - use eg as preview in manual pitch correction; morph() - tempEffects; streamline saving all plots a la ggsave: filename, path, different supported devices instead of only png(); automatic addition of pitch jumps at high temp in soundgen() (?)
