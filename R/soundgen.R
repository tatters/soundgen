# TODO: check functions addAM (input type, saveAudio), addFormants (input type); analyze - move SPL_measured and Pref into loudness = list(), interpolWin, interpolTol, interpolCert becomes interpol = list(); compressor, getRMS - add savePlots; flatSpectrum - update, input type etc; modulationSpetrum - update description; check optimizePars; playme - accept Wave; reverb - add saveAudio, maybe always return detailed list; schwa value - format as in analyze (bullets); transplantEnv, transplantFormants - check input type;
